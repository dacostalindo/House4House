...
## `scripts/wiki_health.py` mechanical linter — shipped Phase 7b as `/wiki-reconcile` skill (2026-05-12)

**Outcome:** the deferred mechanical linter shipped, but in a different shape than the Phase 4 plan anticipated:

- **NOT** as a standalone Python script (`scripts/wiki_health.py`).
- **NOT** with a structured `wiki/_schema.yaml` source of truth.
- **NOT** wired into Makefile / pre-commit / CI as a BLOCKING gate.

Instead, the mechanical lint shipped as a **pure-prose Claude Code skill** at `.claude/skills/wiki-reconcile/SKILL.md`, modeled directly on [obsidian-second-brain's `obsidian-reconcile.md`](https://raw.githubusercontent.com/eugeniughelbur/obsidian-second-brain/main/commands/obsidian-reconcile.md). Five parallel subagents (schema / links / reciprocity / freshness / ingest) scan the wiki and apply rules from `wiki/CLAUDE.md` directly. The skill also folds in the planned `/wiki-import-gstack` workflow (post-merge gstack-artifact ingest) as one of its five subagents.

**Architectural pivots that got us here:**

1. **Structured schema dropped (Phase 7a planning, 2026-05-12).** The DRY argument for `wiki/_schema.yaml` didn't survive scrutiny — rules change rarely; prose + machine surface need updating together regardless; a sentinel test adds complexity without real signal benefit. Rules now live in `wiki/CLAUDE.md` prose + SKILL.md subagent prompts.
2. **Python helper dropped (Phase 7b planning, 2026-05-12).** Initial 7b draft included `.claude/skills/wiki-reconcile/bin/wiki-health.py`. After surveying the actual obsidian-reconcile.md command (~75 lines, parallel-subagent prose, no Python), pivoted to pure-prose skill. The wiki is small (~85 pages); LLM traversal is reliable at this size.
3. **Skill-only scope (locked by user 2026-05-12).** No CI gate, no Makefile target, no pre-commit hook. Same pattern as wiki-lint.

**Real wiki drift surfaced during the original Phase 4 prototype run** (fixed during Phase 4 PR, kept fixed):

- `wiki/concepts/medallion-layering.md` `[[staging-yaml-conventions]]` reference fixed.
- `wiki/sprints/sprint-dev-tooling.md` `[[dev-tooling-design]]` reference fixed.
- `wiki/use-cases/UC-1.md` `[[2026-05-08-streamlit-keplergl-not-superset]]` corrected to `[[2026-05-10-metabase-streamlit-not-superset]]`.

These catches validated the linter approach. The skill now provides ongoing coverage.

**Status:** closed 2026-05-12. See `.claude/skills/wiki-reconcile/SKILL.md` and [[2026-05-12-wiki-linter-deferred-to-phase-7]] for the architectural history.
