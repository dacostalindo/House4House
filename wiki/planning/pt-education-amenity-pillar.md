...
- [~] ~~`listing_school_features` — per-listing nearest-school + best-score features~~ — **dropped 2026-06-09 (PR-F #66 closed as obsolete).** Built once and verified (3,482 rows / 120s build, coverage 99.7% KG @5km down to 47% higher_ed ranking @2km), then deprecated the same day: the actual use case is **interactive polygon-draw spatial queries** against `dim_school` + `fact_school_ranking`, not per-listing pre-materialized columns. Pre-computing 15 columns × every listing is wasted state when the user-facing question is "tell me about THIS area" (drawn on a map) rather than "tell me about this listing". The Leaflet polygon-draw POC is the v1 surface. Mart can be revived if/when a non-interactive consumer (search ranking, lead scoring) needs the pre-materialized columns. **Phase 2 now closes at fact_school_ranking.**
