...
| Phase 6 | `ty` advisory type-check (Astral's static type-checker for Python; advisory-only; graduates to BLOCKING per 3 concrete triggers in TODOS.md) | ✅ done | 2026-05-12 | PR #25 shipped `ty>=0.0.1,<0.1.0` in dev group + `::group::ty` CI step + `make verify` integration. Followup PR added `[tool.ty.src.exclude]` safety net + `unexport VIRTUAL_ENV` Makefile workaround + Phase 6 ADR. Live findings (post-VIRTUAL_ENV-fix): 319 diagnostics (287 errors + 32 warnings), 172 confirmed Airflow `XComArg` false positives. ADR: [[2026-05-12-phase-6-ty-advisory]]. |
| Phase 7 | Claude Skills suite (a/b/c). 7a: Karpathy 4 rules into root CLAUDE.md. 7b: `/wiki-reconcile` skill — folds mechanical lint + interactive drift resolution + post-merge gstack ingest into one pure-prose skill (replaces the originally-planned `scripts/wiki_health.py` + `wiki/_schema.yaml` + separate `/wiki-import-gstack`). 7c: 3 scaffolding skills — `/add-gis-source` + `/add-portal-source` + `/stg-from-bronze`. All skills follow the obsidian-second-brain command pattern (single SKILL.md, ~75 lines, Cosmos frontmatter, parallel subagents where applicable, AI-first compliance mandate). | ✅ done | 2026-05-12 | PR #27 (7a) + PR #28 (7b) + this PR (7c). ADRs: [[2026-05-12-wiki-linter-deferred-to-phase-7]] (architectural reasoning for the schema-YAML drop), [[2026-05-12-pre-commit-local-hook]] (sibling pattern). |
...
- 2026-05-12: Phase 6 → done. PR #25 shipped `ty` advisory type-check (Astral's beta type-checker) wired into `make verify` + CI via Phase 4's `::group::` annotation pattern. Followup PR added `[tool.ty.src.exclude]` safety net + `unexport VIRTUAL_ENV` Makefile workaround + the post-merge wiki ingest. Live findings (post-VIRTUAL_ENV-fix): 319 diagnostics (287 errors + 32 warnings, 172 confirmed Airflow `XComArg` false positives — rest mostly same pattern in DAG templates). Three concrete graduation triggers locked in TODOS.md. ADR: [[2026-05-12-phase-6-ty-advisory]]. Live DX score: 8/10 (boomerang vs plan-devex-review 8.5/10 baseline — slight drop from README staleness + finding-aggregation gap, both fixed in this followup).
- 2026-05-12: Phase 7 → done. PR #27 (7a) added Karpathy 4 rules section to root CLAUDE.md and removed the post-merge gstack-ingest convention prose (now folded into `/wiki-reconcile`). PR #28 (7b) shipped `/wiki-reconcile` skill (mechanical lint + interactive drift resolution + post-merge gstack ingest in one workflow) — pure-prose markdown modeled on [obsidian-reconcile.md](https://github.com/eugeniughelbur/obsidian-second-brain/blob/main/commands/obsidian-reconcile.md). This PR (7c) shipped 3 scaffolding skills: `/add-gis-source` (mirrors 17 GIS sources), `/add-portal-source` (mirrors 6 portals), `/stg-from-bronze` (mirrors 21+ staging models). All skills follow Cosmos frontmatter + obsidian-style numbered procedural workflow. Skill-only (no Python helpers, no CI, no Makefile). Phase 7 closes the dev-tooling roadmap's skill backlog.
