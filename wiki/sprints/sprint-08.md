...
- 2026-05-14: Activity 6 (SRUP constraint plumbing) **done** — PR 1 (`17b7462`: constraint model + `dim_constraint_severity`) + PR 2 (`abed6f5`: 14 `stg_srup_*` staging models with full `properties` unpacking + `srup-properties-schema.md` + GIST verify). `cos_ogc` / `crus_ogc` national bronze loaders OOM on the whole-GeoJSON `json.load` → **deferred to [[sprint-09]]** (user decision); national `cos_ogc` ingestion itself is verified working.
...
- 2026-05-15: **Activity 7 (SCE geocoding) done** — 5 phases, 5 commits (`d6513b6` address-norm + 106 tests, `5e35f41` Nominatim helper + 11 tests, `8e80a32` `sce_geocode` DAG + chain insertion, `c47eeec` `stg_sce_certificates` LEFT JOIN). End-to-end run: 68 min, 55,766 distinct doc_numbers, **100% coverage on Aveiro concelho** (5,718/5,718 docs); 83.78% on Aveiro distrito with documented post-2013-union-freguesia gap **deferred to [[sprint-09]]**. **Activity 8 (silver_sce_buildings skeleton) done** — committed `6724c2a` (empty schema + 4 indexes + fuzzystrmatch extension installed; sprint-09 Slice B body-fills). Activity 9 (pgTAP CI runner) still planned.
- 2026-05-15 (later): **Activity 9 (pgTAP CI runner) done** — committed `f847f87` (CI image swap to `postgis/postgis:16-3.4` + pgTAP install + `pg_prove` step with green-empty guard + `tests/sql/.gitkeep`); follow-ups `520fb84` (drop unused noqa `BLE001`), `ce726f7` (ruff-format auto-fixes), `19ba95a` (pyproject `pythonpath = ['.']` for CI's `pipelines.*` imports). PR [#30](https://github.com/dacostalindo/House4House/pull/30) green at run 25939105896 — `CREATE EXTENSION pgtap` succeeded, `pg_prove --version` ran, the empty-test step logged exactly: `"No tests/sql/*.sql files yet — sprint-09 adds them (Activity 9 green-empty state)"`. **Sprint-08 ship complete** — all 9 activities done (Activity 5 deferred to sprint-09 by design). Two carry-overs tracked in sprint-09: cos_ogc/crus_ogc national bronze-loader OOM fix + post-2013 freguesia-union mapping.
