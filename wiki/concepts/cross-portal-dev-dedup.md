...
1. **Identity surface adds `dev_uids[]`.** The volatile `dev_key = MIN(member_id)` stays as a per-run grouping label (renamed `component_id` for clarity), but the row now also carries `dev_uids UUID[]` — the stable identifiers downstream LLM dev-actor enrichment FKs against. (Originally scoped under [[use-cases/archive/UC-4|UC-4]], archived 2026-06-11; the Project Actors track now lives in the Knowledge-graph-PoC and the surface is reusable for any future per-dev enrichment table.) The `dev_uids` array is sourced from `silver_dev_uid_map`, an append-only `(portal, portal_dev_id) → dev_uid` table. See [[dev-uid-stability]] for the full mechanics, including merge (multiple `dev_uids` in one row) and split (same `dev_uid` in multiple rows) observability.
