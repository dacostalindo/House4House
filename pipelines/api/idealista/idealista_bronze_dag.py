    ...
    def idealista_bronze_load():
        ...
        def create_table() -> None:
            ...
            try:
                ...
                # Add _concelho column if missing (migration for existing tables)
                cur.execute("""
                    ALTER TABLE bronze_listings.raw_idealista
                    ADD COLUMN IF NOT EXISTS _concelho TEXT
                """)
                ...
                for idx_sql in [
                    ...
                    "CREATE INDEX IF NOT EXISTS idx_idealista_concelho ON bronze_listings.raw_idealista(_concelho)",
                ]:
                    cur.execute(idx_sql)
            ...
            try:
                ...
                insert_sql = """
                    INSERT INTO bronze_listings.raw_idealista (
                        ...
                        _property_id, _distrito, _concelho, _operation,
                ...
                # Collect all rows first, before any DELETE
                file_rows: list[tuple[str, list[tuple]]] = []
                for file_info in minio_files:
                    ...
                    try:
                        ...
                        log.error("[idealista] %s: failed to read MinIO file: %s", segment_key, exc)
                        continue

                    rows = []
                    for line_num, line in enumerate(raw_bytes.decode("utf-8").split("\n"), 1):
                        line = line.strip()
                        if not line:
                            continue
                        try:
                            detail = json.loads(line)
                            # Inject path-derived metadata so _map_detail_to_row has correct values
                            detail.setdefault("_operation", operation)
                            detail.setdefault("_distrito", distrito)
                            detail.setdefault("_concelho", concelho or "")
                    ...
                    file_rows.append((segment_key, rows))

                # All files parsed — now DELETE + INSERT in one transaction
                cur.execute(
