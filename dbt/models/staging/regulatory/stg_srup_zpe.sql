...
-- SRUP Rede Natura 2000 — Zonas de Proteção Especial (birds). OGC source,
-- 10 properties keys (fully unpacked below; `fid` duplicates feature_id).
-- See wiki/concepts/srup-constraint-model.md + srup-properties-schema.md.

SELECT
    feature_id,
    'ZPE'::TEXT                             AS constraint_code,
    ...
    _source_url,
    _load_timestamp
FROM {{ source('bronze_regulatory', 'raw_srup_zpe') }}
